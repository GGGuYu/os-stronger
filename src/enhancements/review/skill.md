...
1. **STEP 0 tier 解析 + 熔断**:从 Review task 文字解析 `[tier=XXX]` → `maxCycle = low?2:3`。`lastCompleted >= maxCycle` → 询问用户 archive,不启动子 agent
2. **Check**: `.os-stronger/review-guide.md` exists? (boolean check — do NOT read its contents)
3. **Write**: requirement summary to `.os-stronger/requirement-summary.md`
4. **Review**: launch review subagent (pass file paths, not contents)。**max 档 cycle 1**:起两个独立子 agent(并行优先否则串行),主 agent 融合两份 findings 交叉
5. **Evaluate**: 按 tier 严格度判断每条 finding — low 全程"属实且值得修";high/max cycle 1 严格倾向修、cycle 2+ 正确性为主
6. **Fix**: create `Review N Fix - <desc>` tasks in tasks.md for accepted findings
7. **Cycle**: `currentCycle < maxCycle` 有 fix → 加 `Review [tier=同] N+1` task;`currentCycle === maxCycle` 有 fix → 熔断,询问用户 archive;无 fix → 询问用户 archive
...
- goal 与 review 互斥安装(init 时二选一),不共存

...
**触发方式**(两层):
1. **主触发**:propose patch 要求 propose 子 agent **先问 review 档位**(low/high/max,默认 low;优先 AskUserQuestion,不可用就文本问),把 `[tier=XXX]` 写进 tasks.md 末尾的 `- [ ] Review [tier=XXX]: 按 apply skill 指导启动 Review 1`。agent 走到这个 task 时 CLI 直接推到面前,不依赖长上下文记忆。**问了没明确答复(沉默/含糊/跑题)→ 立即 default low 继续,不阻塞流程**(propose 完本就会停,是问的天然窗口;用户没专门答复档位就走 low)。

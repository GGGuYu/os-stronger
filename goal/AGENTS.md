...
**熔断语义**：`maxFixCycles=2` 意味着最多 2 轮 fix→test 循环。第 3 轮 test 失败时 `cycle=3 > 2` → 熔断。
...
| 改 fixFlow 的熔断判断条件 | ⚠️ 谨慎 | `cycle > maxCycles` 是有意为之。maxCycles=2 允许 2 轮 fix，第 3 轮失败才熔断 |
...
3. **fix 分析依赖子 agent 质量**：test 失败后需要分析子 agent 确定修什么。如果分析子 agent 判断失误，fix 可能不对。但这是所有 agent 方案的通病——靠 fresh context + 明确提示词缓解。

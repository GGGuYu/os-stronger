...
2. instructions.js 子 agent 提示词（subagentPrompt）：propose 提示词引用 openspec-propose，apply 提示词引用 openspec-apply-change + openspec-archive-change
3. instructions.js 主 agent 指令（nextAction.instruction）：propose_next 引用 openspec-propose，apply_next 引用 openspec-apply-change + openspec-archive-change，fix_analysis_needed 引用后续的 propose/apply 流程
...
### 决策 14：动态 change 编排 + 拆分粒度

**选择**：change 列表不是一开始定死的。主 agent 可以在前置 change 完成后，根据其产物追加后续 change。同时给 `change add` 加 `--before` 参数和智能默认，让"追加中间 change"有趁手工具。

**动态编排的机制**：
- `addChange` 支持 `--before <id>`：把新 change 插在指定 change 之前（锚点必须存在且未归档）。
- **智能默认**：不加 `--before` 时，normal change 自动插到最后一个**未归档** testchange 之前（覆盖"前置完成、追加中间 change"的主场景，避免新 change 排到 testchange 后面导致 testchange 被先执行）。
- test/fix change 或无 testchange 时：push 末尾（兼容旧行为）。
- 锚点已归档时报错——已归档的 change 不再参与执行顺序，不能作插入位置。
- `deleteChange` 只允许删 **skeleton** 阶段的 change：编排调整时删掉不想要的未动骨架。proposed/archived 的 change 在 OpenSpec 侧已有目录或已合并 specs，删不干净，故拒绝——方向不对的"做了一半"的 change 走 block 让它停下，或让它走失败进 fix/熔断，再重新拆，而非强删。**这是有意的不对称：追加安全（state 操作 + 智能默认），删除严格（只能删未动的）**——因为追加是新增，删除是回退，回退在分布式/有副作用系统里永远比新增危险。

**为什么需要动态编排**：有些 change 的拆分依赖前置 change 的产物。典型：视频制作（文字稿→SRT→分镜→分镜实现→...）。文字稿没出来，不知道 SRT 多少段；SRT 没出来，不知道几个分镜。一开始就把所有 change 写死要么过粗要么过细。正确做法：先注册确定的前置 change + testchange，前置完成后追加中间 change。

**为什么是 append 到中间（--before testchange）而非末尾**：一开始注册的顺序就是执行顺序（`remainingChanges[0]` 按数组取第一个未归档的）。如果新 change push 末尾，会排到 testchange 后面，状态机会先执行 testchange——错误。必须插在 testchange 之前。

**goal.md 随之演化**：每个 change 完成后，主 agent 更新 goal.md 的 `## 宏观架构` 章节（把"暂定计划"改成"实际产物 + 新理解"）。goal.md 是**活文档**，但仅演化宏观层，不写实现细节（那是 change 产物的职责，决策 8）。goal.md 的 change 计划是"当前理解"，`state.changes` 是"执行真相"——以 state 为准，goal.md 标注"计划会演化"防子 agent 混淆。

**拆分粒度原则**（同时适用于普通 change 和 fix change）：
- **别拆太细**：一个 change 是一个连贯工作单元，不是单个 task。服务于同一目的的几个小任务写进同一个 change 的 tasks.md，不要每个 task 开一个 change。
- **模块级才拆**：只有当部分是真正独立的模块、各自需要独立设计、分开能让 fresh-context 子 agent 上下文更清晰时，才拆成多个 change。
- **判断标准**：能用 handful of tasks 说清并做完的 → 一个 change。跨多个明确分离的模块、每个都要独立设计的 → 多个 change。
- **小目标也合法**：1 个实现 change + 1 个 testchange（1+1）是完全正当的用法——你仍得到 testchange 的语义评估 + 测试作为内置审查。别给小目标硬凑 change。

**红线**：
- 不要把动态编排做成"一开始不注册 testchange"。testchange 必须一开始就注册（决策 4），动态追加的是中间的 normal change。
- 不要让新 normal change 排到 testchange 后面。智能默认已处理，但 `--before` 锚点选错也可能错位——主 agent 用 `--before` 时要确认锚点。
- 不要为粒度加 state 字段或 CLI 参数（除了 `--before` 这个位置控制）。粒度是主 agent 的判断，靠 skill.md 的指导，不靠程序强制。
- 不要把"动态编排"和"中途改已注册 change 的顺序"混为一谈。动态编排 = 追加新 change 到正确位置 / 删除未动的 skeleton change；已注册且未开始的 change 的顺序不应改动（会让依赖关系混乱）。
- 不要让 `deleteChange` 支持删 proposed/archived 的 change。已 proposed 的 OpenSpec 侧有目录、已 archived 的已合并 specs，删不干净。方向不对的进行中 change 走 block 或让它失败进 fix/熔断，绝不强删。
